...
- 原始 Excel 字段名是事实表与台账表的唯一真相。
- `create_time` 是墒情事实表查询执行统一使用的时间列。
- 墒情域不再保留批次语义文档与批次表说明。
- 文档必须与 `infra/mysql/init/` 下的建表脚本保持一致。

## 权威来源

- 基础建表与索引：`infra/mysql/init/001_init_tables.sql`
- 规则与模板初始化：`infra/mysql/init/002_insert_data.sql`
- 墒情事实与地区别名种子：`infra/mysql/init/003_insert_soil_data.sql`
- 审计列扩展：`infra/mysql/init/004_add_audit_columns.sql`
- 设备台账表：`infra/mysql/init/005_add_subject_device_record.sql`
