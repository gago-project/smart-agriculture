# 9. Deterministic 查询治理与 `QueryProfile` 收口方案

> 本文档用于解释：为什么当前一些问题看起来像“不断补词、不断补 Case”，以及后续应该如何用统一抽象把 deterministic 查询链路收口成稳定结构，而不是继续打补丁。
...
- 多轮上下文已经有 `query_state + time_window + resolved_entities`

但还存在两个没有完全收口的语义维度：

| 维度 | 当前状态 | 直接后果 |
|---|---|---|
| `data_focus` | 还没有作为一等公民统一表达 | “普通墒情” 和 “只看预警/异常后的墒情” 经常混用 |
| `answer_mode` | 还没有作为统一契约贯穿路由、执行、渲染 | “概况 / 数量 / 列表 / 详情” 之间容易漂移 |

这两个维度一旦散落在不同函数里，就会出现典型问题：
...
换句话说，当前系统已经有“中心路由”的雏形，但还缺“中心查询画像”。

## 3. 治理目标

治理目标不是继续给某个分支补关键词，而是把 deterministic 查询链路统一收口成一个内部对象：
...
## 4. 当前 `QueryShape` 和未来 `QueryProfile` 的关系
