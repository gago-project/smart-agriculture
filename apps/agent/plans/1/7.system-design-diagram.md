...
flowchart TD
    ...
    P --> Q["Next BFF<br/>/api/admin/agent/query-evidence"]
    Q --> O
```
...
- `QueryLogRepository`：查询审计单一真源，保存每次 tool 调用的 `query_id / executed_sql_text / executed_result_json`
- `Next BFF /api/admin/agent/query-evidence`：管理员聊天页右侧证据接口，按 `session_id + turn_id` 回拉当轮查询证据
...
sequenceDiagram
    ...
    participant WEB as Next Web(Admin)
    participant BFF as Query Evidence BFF
    ...
    alt 非业务 / 越界 / 结束语
        ...
        loop LLM ↔ 工具调用（最多 5 次）
            ...
            alt LLM 返回 tool_call
                ...
                DB-->>EXEC: 查询结果
                EXEC-->>LOOP: {records: [...]}
                LOOP->>FLOW: 生成 query_log_entry(session_id:turn_id:tool_index + SQL 审计 + 结果 JSON)
        ...
        FLOW-->>API: 最终响应
        API-->>U: 返回可核验回答
        WEB->>BFF: GET /api/admin/agent/query-evidence?session_id=&turn_id=
        BFF->>DB: SELECT agent_query_log by session_id + turn_id
        DB-->>BFF: executed_sql_text / executed_result_json / missing_fields
        BFF-->>WEB: 右侧证据数据
    end
```
...
- `QueryLogRepository`：把“本轮到底查了什么 SQL、回来了什么结果”固化为可追溯审计记录
...
- 管理员右侧证据侧栏：完整数据走接口实时查询，本地只保存选中态，不把整包结果长期塞进 localStorage
