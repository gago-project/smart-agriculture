...
flowchart TD
    ...
    C --> D["InputGuard"]
    C --> E["Intent & Slot Extractor<br/>确定性优先 / Qwen 可选"]
    ...
    C --> N["ResponseGenerator<br/>确定性优先 / Qwen 可选润色"]
    C --> O["DataFactCheck + AnswerVerify"]
    C --> P["FallbackGuard"]

    J --> Q["MySQL<br/>fact_soil_moisture / region_alias / warning_template"]
...
- `Qwen / 千问`：可选增强，只在确定性解析需要兜底或确定性回答需要受控润色时参与
...
sequenceDiagram
    ...
    alt 非业务 / 乱输入 / 越界问题
        ...
        FLOW->>FLOW: 确定性意图识别 + 槽位抽取
        opt 确定性结果仍需澄清且 Qwen 可用
            FLOW->>QWEN: 兜底结构化解析
...
- 地区 / 设备：来自 `RegionAliasResolver + region_alias(city/county) + fact_soil_moisture 存在性校验`
