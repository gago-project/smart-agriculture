...
本目录是一套围绕「墒情（土壤墒情）Agent MVP」的配套文档，编号 `1`～`8` 表示推荐阅读与落地顺序：**业务与总方案 → 业务对齐表 → 验收矩阵 → Python 工程与实施 → 伪代码 → 架构图 → 风险契约**。实现代码在 `apps/agent` 中演进时，可对照这些文件核对范围与验收口径。
...
- **快速建立全局认识**：读 **1** → **7**。  
- **和业务方对齐「怎么答」**：**2**。  
- **开发与验收**：**3** 与 **5** 对照实现；**4**、**6** 解决结构与写法；上线前用 **8** 做风险与契约检查。
