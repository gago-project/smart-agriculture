...
test('soil admin paginated listing avoids prepared execute for LIMIT/OFFSET', () => {
  assert.match(
    repositorySource,
    /const \[rows\] = await connection\.query\([\s\S]*ORDER BY create_time DESC[\s\S]*LIMIT \? OFFSET \?/,
  );
});
...
test('admin routes require admin role instead of generic authenticated user', () => {
  const adminRoutes = [
    ...
    '../app/api/admin/soil/import-preview/route.ts',
    '../app/api/admin/soil/import-preview/[previewToken]/diff/route.ts',
    '../app/api/admin/soil/import-preview/[previewToken]/apply/route.ts',
