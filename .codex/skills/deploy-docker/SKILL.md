...
- If `docker compose up` fails due to port conflict on `3000`, the process-mode web was not fully stopped. Find and kill it first.
- If the agent container exits immediately, check `docker compose logs agent` for startup errors.
- If the domain returns 502, verify the Docker web container is healthy and nginx is proxying to `3000`.
